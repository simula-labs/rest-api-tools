...
var defKey2defName = function (key) {
    return "".concat(key[0].replace(/^([^a-zA-Z$_])$/, "$$$1").toUpperCase()).concat(key
        .slice(1)
...
var $ref2Type = function (ref) {
    var _a = (0, exports.$ref2TypeName)(ref), typeName = _a.typeName, propName = _a.propName;
    return "Types.".concat((0, exports.defKey2defName)(typeName)).concat(propName ? "['".concat(propName, "']") : "").replace(/Array$/, "[]");
};
...
var isObjectSchema = function (schema) { return !(0, exports.isRefObject)(schema) && schema.type !== "array"; };
exports.isObjectSchema = isObjectSchema;
var getPropertyName = function (name) {
    return /^[a-zA-Z_$][0-9a-zA-Z_$]*$/.test(name) ? name : "'".concat(name, "'");
};
exports.getPropertyName = getPropertyName;
var of2Values = function (obj, required) {
    var values = (obj.oneOf || obj.allOf || obj.anyOf || [])
        .map(function (p) { return (0, exports.schema2value)(p, required); })
...
var object2value = function (obj, required) {
    ...
    var value = Object.keys(properties)
        ...
        .map(function (name) {
        var _a, _b;
        var val = (0, exports.schema2value)(properties[name], required);
        if (!val)
            return null;
        return {
            name: (0, exports.getPropertyName)(name),
    ...
    if (additionalProps) {
        var val = additionalProps === true
            ...
            : (0, exports.schema2value)(additionalProps, required);
        if (val)
            value.push({
                name: "[key: string]",
                required: required,
...
var schema2value = function (schema, required, isResponse) {
    ...
    if ((0, exports.isRefObject)(schema)) {
        ...
        if (schema.oneOf || schema.allOf || schema.anyOf) {
            ...
            isEnum = true;
            value = schema.type === "string" ? schema.enum.map(function (e) { return "'".concat(e, "'"); }) : schema.enum;
        }
        else if (isArraySchema(schema)) {
            isArray = true;
            value = (0, exports.schema2value)(schema.items, required);
        }
...
var path2requestPath = function (path) { };
